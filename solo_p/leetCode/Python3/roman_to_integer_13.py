...
def roman_to_int(s):
    ...
    # Iterate through each symbol in the Roman numeral string
    for i in range(len(s)):
        # Check if the current symbol is smaller than the next symbol
        if i < len(s) - 1 and symbol_values[s[i]] < symbol_values[s[i + 1]]:
            # If so, subtract the current symbol value from the total
            total -= symbol_values[s[i]]
        else:
            # Otherwise, add the current symbol value to the total
            total += symbol_values[s[i]]
    
    return total  # Return the final converted integer

# Test the function with example Roman numerals
print(roman_to_int("III"))  # Output: 3
print(roman_to_int("LVIII"))  # Output: 58
print(roman_to_int("MCMXCIV"))  # Output: 1994
print(roman_to_int("MCMXVXVXCIV"))  # Output: 2004
